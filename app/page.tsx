    ...
    try {
      const emailResponse = await fetch("https://formspree.io/f/mreokqyd", {
        ...
        body: JSON.stringify({
          ...
          subject: "Summer Outing Registration",
...
Phone number: ${phone}`,
      ...
      const whatsappMessage =
        ...
        `Name: ${name}%0A` +
        `Email: ${email}%0A` +
        `Number of people: ${peopleCount}%0A` +
        `Phone number: ${phone}`;
      ...
      setStatusMessage("Something went wrong sending the form. Please try again.");
    } finally {
      setIsSending(false);
    }
  }
      ...
      <section className="relative">
        ...
        <div className="absolute inset-0 bg-black/35 flex items-center justify-center">
          <div className="text-center px-6 max-w-4xl">
            <h1 className="text-4xl md:text-6xl font-extrabold text-black bg-white/80 px-6 py-3 rounded-xl inline-block">
              Sureway Assemblies of God
            </h1>
            <div className="mt-4">
              <p className="text-xl md:text-2xl font-semibold text-black bg-white/80 px-6 py-3 rounded-xl inline-block">
      ...
      <section className="max-w-5xl mx-auto px-6 py-12">
        <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
            <TimeBox label="Days" value={mounted ? timeLeft.days : 0} />
            <TimeBox label="Hours" value={mounted ? timeLeft.hours : 0} />
            <TimeBox label="Minutes" value={mounted ? timeLeft.minutes : 0} />
            <TimeBox label="Seconds" value={mounted ? timeLeft.seconds : 0} />
          </div>
        </div>
      </section>
